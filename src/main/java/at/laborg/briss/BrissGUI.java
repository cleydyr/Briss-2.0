...
public class BrissGUI extends JFrame implements PropertyChangeListener, ComponentListener {
	...
	private void tryToLoadFileFromArgument(String[] args) {
		...
		if (fileArg.exists() && fileArg.getAbsolutePath().trim().endsWith(".pdf")) { //$NON-NLS-1$
			try {
				...
				JOptionPane.showMessageDialog(this, e.getMessage(), Messages.getString("BrissGUI.brissError"), //$NON-NLS-1$
	...
	private void init() {
		...
		showHelpButton.addActionListener(
				a -> new HelpDialog(this, Messages.getString("BrissGUI.brissHelp"), Dialog.ModalityType.MODELESS));
	...
	private void loadPDF(File file) {
		try {
			...
			JOptionPane.showMessageDialog(this, e.getMessage(), Messages.getString("BrissGUI.loadingError"), //$NON-NLS-1$
					JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void showPreview() {
		try {
			...
			JOptionPane.showMessageDialog(this, e.getMessage(), Messages.getString("BrissGUI.croppingError"), //$NON-NLS-1$
					JOptionPane.ERROR_MESSAGE);
		} finally {
			setIdleState(); // $NON-NLS-1$
		}
